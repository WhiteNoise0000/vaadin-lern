...
var nicEditorInstance = bkClass.extend({
	...
	construct : function(e,options,nicEditor) {
		...
		if(isTextarea || this.options.hasPanel) {
			...
			var editorElm = new bkElement('DIV').setStyle({width : '99%', margin: '4px', minHeight : '150px'}).addClass('main').appendTo(this.editorContain);
			...
			editorElm.setStyle((ie7s) ? {height : '30%'} : {overflow: 'hidden'});
			this.elm = editorElm;	
		}
...
var nicEditorIFrameInstance = nicEditorInstance.extend({
	...
	init : function() {	
		...
		this.elmFrame = new bkElement('iframe').setAttributes({'src' : 'javascript:;', 'frameBorder' : 0, 'allowTransparency' : 'true', 'scrolling' : 'no'}).setStyle({height: '200px', width: '100%'}).addClass('frame').appendTo(this.elm);
